...
function renderSkills() {
  ...
  // If the wrapper replaced the original container, find it
  if (!container) {
    container = document.querySelector(".skills-wrapper");
    if (container) {
      // Re-create the original container for desktop
      const original = document.createElement("div");
      original.className = "skills-grid";
      original.id = "skills-container";
  ...
  if (isMobile && skills.length > 4) {
    const topSkills = skills.slice(0, 8);
    const bottomSkills = skills.slice(8);

    const wrapper = document.createElement("div");
    wrapper.className = "skills-wrapper";
    wrapper.id = "skills-container";

    const topGrid = document.createElement("div");
    topGrid.className = "skills-grid";

    const bottomGrid = document.createElement("div");
    bottomGrid.className = "skills-grid-bottom";

    topSkills.forEach((skill) => {
      const card = document.createElement("article");
      card.className = "skill-card";
      card.innerHTML = `<img src="${skill.iconPath}" alt="${skill.label}" class="skill-icon-image" loading="lazy" />`;
...
function renderProjectCard() {
  ...
  // Special handling for Ongoing Project
  if (project.key === "ongoing") {
    image.src = "";
    image.style.display = "none";
    
    // Create coming soon placeholder if it doesn't exist
    ...
    // Hide coming soon placeholder for other projects
